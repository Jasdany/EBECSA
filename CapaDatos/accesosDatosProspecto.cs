   ...
   public class accesosDatosProspecto
    {
        ...
        public List<Prospecto> listarProspecto()
        {
            try
            {
                ...
                cnx.Open();
                dr = cm.ExecuteReader();
                listaProspecto = new List<Prospecto>(); //Lista de cuentas
                while (dr.Read()) //Recorrer cada registro
                {
                    ...
                    p.TelefonoP = dr["TelefonoP"].ToString();
            ...
            { cm.Connection.Close(); }
            return listaProspecto; //regresa lista de registros
        ...
        public int EditarProspecto(Prospecto pr)
        {
            ...
            {
                SqlConnection cnx = cn.conectar(); //Conexion
                cm = new SqlCommand("Prospecto", cnx);
                ...
                cm.Parameters.AddWithValue("@TelefonoP", pr);
                cm.CommandType = CommandType.StoredProcedure;
                cnx.Open();
                cm.ExecuteNonQuery();
                indicador = 1;
